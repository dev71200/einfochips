...
class TestMainClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for import_name, mocked_object in [("print_info", self.mocked_printInfo),
                                           ...
                                           ("einfoReport", self.mocked_report),
                                           ("webbrowser", self.mocked_browser)]:
            constructor_obj = patch("einfochips.__main__.%s" % import_name, return_value=mocked_object).start()
    ...
    async def test_empty(self):
        ...
        with patch("sys.stderr", return_value=MagicMock()):
            with self.assertRaises(SystemExit):
                args = einfochipsArgumentParser().parse_args(args)
    ...
    async def test_aws_provider(self):
        ...
        assert (report_init_args[2] == "einfochips-report")  # report_dir
