...
import unittest
from einfochips.core.console import set_logger_configuration, print_debug
from einfochips.core.rule import Rule
from einfochips.core.ruleset import Ruleset


class TesteinfoRulesRuleset(unittest.TestCase):
    ...
    @mock.patch("einfochips.core.ruleset.print_error")
    ...
    @mock.patch("einfochips.core.ruleset.print_exception")
    ...
    def test_path_for_cloud_providers(self):
        ...
        assert (os.path.samefile(target.rules_data_path, './einfochips/providers/aws/rules'))

        target = Ruleset(cloud_provider='azure', filename=self.test_ruleset_001)
        assert (os.path.samefile(target.rules_data_path, './einfochips/providers/azure/rules'))

        target = Ruleset(cloud_provider='gcp', filename=self.test_ruleset_001)
        assert (os.path.samefile(target.rules_data_path, './einfochips/providers/gcp/rules'))

    def test_path_for_ruletypes(self):
        rpath = "./einfochips/providers/aws/rules/"
    ...
    @mock.patch("einfochips.core.ruleset.prompt_yes_no")
