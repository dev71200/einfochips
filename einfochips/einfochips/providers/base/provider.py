...
from einfochips import __version__ as einfo_version
from einfochips.core.console import print_exception, print_info, print_error
from einfochips.output.html import einfoReport
from einfochips.providers.base.configs.browser import get_object_at
...
class BaseProvider:
    ...
    Root object that holds all of the necessary provider-specific resources and einfo configuration items.
    ...
    async def fetch(self, regions=None, excluded_regions=None, partition_name=None):
        ...
        """
        This is quite ugly but the legacy einfo expects the configurations to be dictionaries.
        Eventually this should be moved to objects/attributes, but that will require significant re-write.
        """
        report = einfoReport(self.provider_code, 'placeholder')
    ...
    def manage_object(self, object, attr, init, callback=None):
        """
        This is a quick-fix copy of Opinel's manage_dictionary in order to support the new einfochips object which isn't
        a dict
        """
        if type(object) == dict:
            ...
            if 'summaries' in self.metadata[service_group]:
                for summary in self.metadata[service_group]['summaries']:
                    for callback in self.metadata[service_group]['summaries'][summary]['callbacks']:
                        ...
                        # quick fix as legacy einfo expects "self" to be a dict
                        target_object = self
