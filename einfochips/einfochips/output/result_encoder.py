...
from einfochips import DEFAULT_REPORT_DIRECTORY
from einfochips.core.console import print_exception, print_info
from einfochips.output.utils import get_filename, prompt_for_overwrite


class einfoJsonEncoder(json.JSONEncoder):
    """
    JSON encoder class
    """
...
class einfoResultEncoder:
    ...
    @staticmethod
    def to_dict(config):
        return json.loads(json.dumps(config, separators=(',', ': '), cls=einfoJsonEncoder))


class SqlLiteEncoder(einfoResultEncoder):
...
class JavaScriptEncoder(einfoResultEncoder):
    """
