...
from einfochips import DEFAULT_REPORT_DIRECTORY, DEFAULT_REPORT_RESULTS_DIRECTORY
from einfochips.core.console import print_error
...
def get_filename(file_type, file_name=None, file_dir=None, relative_path=False, file_extension=None):
    if file_type == 'REPORT':
        ...
        name = f'einfochips_results_{file_name}' if file_name else 'einfochips_results'
        ...
        extension = 'js'
        first_line = 'einfochips_results ='
    elif file_type == 'EXCEPTIONS':
        name = f'einfochips_exceptions_{file_name}' if file_name else 'einfochips_exceptions'
        if not relative_path:
            directory = os.path.join(file_dir if file_dir else DEFAULT_REPORT_DIRECTORY, DEFAULT_REPORT_RESULTS_DIRECTORY)
        else:
            directory = DEFAULT_REPORT_RESULTS_DIRECTORY
        ...
        name = f'einfochips_errors_{file_name}' if file_name else 'einfochips_errors'
        if not relative_path:
            directory = os.path.join(file_dir if file_dir else DEFAULT_REPORT_DIRECTORY, DEFAULT_REPORT_RESULTS_DIRECTORY)
        else:
            directory = DEFAULT_REPORT_RESULTS_DIRECTORY
