...
function hideAll() {
    ...
    $("[id*='.details']").hide()
    var element = document.getElementById('einfo_display_account_id_on_all_pages')
...
function updateNavbar(path) {
    ...
    if (path === '') {
        $('#einfochips_navbar').addClass('active')
...
/**
 * Get the format of the results that einfo chips is reading from
 */
function getFormat() {
...
function loadMetadata() {
    if (getFormat() === resultFormats.json) {
        runResults = geteinfochipsResultsJson()
    } else if (getFormat() === resultFormats.sqlite) {
...
/**
 * Show About einfo chips modal
 */
function showAbout() {
    $('#modal-container').html(about_einfochips_template())
    $('#modal-container').modal()
}
