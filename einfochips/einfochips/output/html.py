...
from einfochips import DEFAULT_INCLUDES_DIRECTORY
from einfochips import DEFAULT_REPORT_DIRECTORY, DEFAULT_REPORT_RESULTS_DIRECTORY, DEFAULT_INCLUDES_DIRECTORY
from einfochips import ERRORS_LIST
from einfochips.core.console import print_info, print_exception
from einfochips.output.result_encoder import JavaScriptEncoder, SqlLiteEncoder
from einfochips.output.utils import get_filename, prompt_for_overwrite
...
class HTMLReport:
    ...
    def __init__(self, report_name=None, report_dir=None, timestamp=False, exceptions=None, result_format=None):
        ...
        self.einfo_report_data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data')
        self.html_data_path = os.path.join(self.einfo_report_data_path, 'html')
    ...
    def prepare_html_report_dir(self):
        ...
        archive = os.path.join(self.einfo_report_data_path, 'includes.zip')
        zip_ref = zipfile.ZipFile(archive)
        zip_ref.extractall(self.report_dir)
        zip_ref.close()
        # Copy static files
        inc_einfo_dir = os.path.join(self.report_dir, DEFAULT_INCLUDES_DIRECTORY)
        src_inc_einfo_dir = os.path.join(self.einfo_report_data_path, DEFAULT_INCLUDES_DIRECTORY)
        if os.path.isdir(inc_einfo_dir):
            shutil.rmtree(inc_einfo_dir)
        shutil.copytree(src_inc_einfo_dir, inc_einfo_dir)


class einfoReport(HTMLReport):
    """
    einfo HTML report
    """
