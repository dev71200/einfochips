import argparse
from einfochips import __version__


class einfochipsArgumentParser:

    def __init__(self):
        self.parser = argparse.ArgumentParser(epilog='To get addtional help on a specific provider run: einfo.py {provider} -h')
        ...
        self.parser.add_argument('-v', '--version',
                                 action='version',
                                 version=f'einfo chips {__version__}')

        self.common_providers_args_parser = argparse.ArgumentParser(add_help=False)

        self.subparsers = self.parser.add_subparsers(title="The provider you want to run einfo against",
                                                     dest="provider")
    ...
    def _init_aws_parser(self):
        parser = self.subparsers.add_parser("aws",
                                            ...
                                            help="Run einfo against an Amazon Web Services account")
    ...
    def _init_gcp_parser(self):
        parser = self.subparsers.add_parser("gcp",
                                            parents=[self.common_providers_args_parser],
                                            help="Run einfo against a Google Cloud Platform account")
        ...
        gcp_auth_modes.add_argument('-u',
                                    '--user-account',
                                    action='store_true',
                                    help='Run einfo with a Google Account')

        gcp_auth_modes.add_argument('-s',
                                    '--service-account',
                                    action='store',
                                    metavar="KEY_FILE",
                                    help='Run einfo with a Google Service Account with the specified '
    ...
    def _init_azure_parser(self):
        parser = self.subparsers.add_parser("azure",
                                            parents=[self.common_providers_args_parser],
                                            help="Run einfo against a Microsoft Azure account")
        ...
        azure_auth_modes.add_argument('-c',
                                      '--cli',
                                      action='store_true',
                                      help='Run einfo using configured azure-cli credentials')

        # username/password authentication
        azure_auth_modes.add_argument('--user-account',
                                      action='store_true',
                                      help='Run einfo with user credentials')
        ...
        azure_auth_modes.add_argument('--user-account-browser',
                                      action='store_true',
                                      help='Run einfo with user credentials, authenticating through a browser (useful when MFA is enforced)')

        # Service Principal authentication
        azure_auth_modes.add_argument('-s',
                                      '--service-principal',
                                      action='store_true',
                                      help='Run einfo with an Azure Service Principal')
        ...
        azure_auth_modes.add_argument('--file-auth',
                                      ...
                                      dest='file_auth',
                                      metavar="FILE",
                                      help='Run einfo with the specified credential file')

        # Managed Service Identity (MSI) authentication
        azure_auth_modes.add_argument('-m',
                                      '--msi',
                                      action='store_true',
                                      help='Run einfo with Managed Service Identity')
    ...
    def _init_aliyun_parser(self):
        parser = self.subparsers.add_parser("aliyun",
                                            parents=[self.common_providers_args_parser],
                                            help="Run einfo against an Alibaba Cloud account")
    ...
    def _init_oci_parser(self):
        oci_parser = self.subparsers.add_parser("oci",
                                                parents=[self.common_providers_args_parser],
                                                help="Run einfo against an Oracle Cloud Infrastructure account")
    ...
    def _init_common_args_parser(self):
        parser = self.common_providers_args_parser.add_argument_group('einfo Arguments')
        ...
        parser.add_argument('--max-workers',
                            ...
                            type=int,
                            default=10,
                            help='Maximum number of threads (workers) used by einfo chips (default is 10)')
        parser.add_argument('--report-dir',
                            dest='report_dir',
                            default=None,
                            help='Path of the einfo report.')
        parser.add_argument('--report-name',
                            dest='report_name',
                            default=None,
                            help='Name of the einfo report.')
